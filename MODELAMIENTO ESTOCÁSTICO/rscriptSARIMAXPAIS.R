...
var(base_pais_pandemia$Casos)

###### Transformación de Box #####

base_pais_pandemia$Casos =  log(base_pais_pandemia$Casos)
var(base_pais_pandemia$Casos)
...
decomp_obj <- decompose(ts_pais_pandemia, type = "additive")
...
ts.plot(as.vector(diff(ts_pais_pandemia)), 
        gpars = list(
...
ts.plot(as.vector(ts_pais_pandemia), 
        gpars = list(
          main = "Evolución de la Serie con Periodo Pandemia",
          ylab = "Cantidad de Atenciones",
          xlab = "Semanas (Índice)",
...
plot_clean <- autoplot(fc_final, PI = FALSE) + 
  ...
  theme_minimal() +
  coord_cartesian(xlim = c(time(ts_test)[1], tail(time(ts_test),1))) 
...
t <- 1:length(ts_pais_pandemia)
dummy_pandemia <- rep(0, length(ts_pais_pandemia))
dummy_pandemia[169:270] <- 1
...
fit_final <- Arima(ts_pais_pandemia,
                   order = c(0, 1, 2),
                   seasonal = c(0, 0, 2),
                   xreg = matriz_xreg_total,
                   lambda = 0,
                   include.constant = FALSE)
