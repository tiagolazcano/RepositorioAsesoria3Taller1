...
DEISESTABLECIMIENTOS <- read_excel("DEISESTABLECIMIENTOS.xlsx")

DEISESTABLECIMIENTOS <- DEISESTABLECIMIENTOS %>% 
...
hosp_ref <- DEISESTABLECIMIENTOS$IdEstablecimiento
...
faltantes$CodigoRegion <- DEISESTABLECIMIENTOS$CodigoRegion[matches]
...
urgenciaIRA2022 <- urgenciaIRA2022 %>% 
  left_join(
    DEISESTABLECIMIENTOS %>% select(IdEstablecimiento, CodigoRegion),
